...
static void irda_device_init(void)
{
	...
	gpio_tlmm_config(GPIO_CFG(GPIO_IRDA_EN,0, GPIO_CFG_OUTPUT,
...
/*
static void cypress_gpio_init(void)
{

	if (system_rev < 9) {
		gpio_tlmm_config(GPIO_CFG(GPIO_TOUCHKEY_SCL, 0, GPIO_CFG_INPUT,
				GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		}
	else {
		gpio_tlmm_config(GPIO_CFG(GPIO_TOUCHKEY_SCL_2, 0, GPIO_CFG_INPUT,
				GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		}
...
{
	gpio_set_value(GPIO_IRDA_EN, onoff);
}
...
static struct sec_jack_zone jack_zones_rev7[] = {
	[0] = {
		.adc_high	= 3,
		.delay_us	= 10000,
		.check_count	= 10,
...
static struct sec_jack_buttons_zone jack_buttons_zones_rev7[] = {
	{
		.code		= KEY_MEDIA,
		.adc_low	= 0,
		.adc_high	= 225,
...
static void sec_jack_init(void)
{
	...
	if (system_rev >= BOARD_REV07) {
		...
		sec_jack_data.buttons_zones = jack_buttons_zones_rev7;
	}
